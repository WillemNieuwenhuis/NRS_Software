...
;
;   OPTION_BOX: Add a checkbox controlling the text box. The initial state of the
;       checkbox is off, and the textbox cannot be editted
;   
;   OPTION_ON: Set the initial state of the checkbox to on; this makes the textbox edittable
;   
...
;
;   unit_text:  A text after the text box, that can be used for observation units like (m, kg)
;   
...
;
;   TSIZE: An explicit horizontal size (in characters) for the label
...
;
;       base = WIDGET_BASE()
;       field = CW_FIELD_EX(base, TITLE="Name:", /FRAME)
...
;
;   Willem Nieuwenhuis, nov 2009: Removed the ROW and COL keywords, now
;         only as row. Added keword TSIZE controlling the width of the label.
;         Added keywords OPTION_BOX and OPTION_ON: OPTION_BOX adds a checkbox
;         in front of the field. It controls the edit state of the text box.
;         The OPTION_ON defines the initial state of the check box (default OFF).
;         Added function to read the state of the checkbox
;-
...
;
FUNCTION cw_field_ex_validate, Value
...
FUNCTION cw_field_ex_value, Value, Type

    COMPILE_OPT hidden

    IF Type EQ 0 THEN RETURN, Value

    NValue  = cw_field_ex_validate(Value[0])
...
;
PRO cw_field_ex_set, Base, Value
    ...
    WIDGET_CONTROL, State.TextId, SET_VALUE=STRTRIM(cw_field_ex_value(sValue, State.Type),2)
...
FUNCTION cw_field_ex_get, Base
    ...
    Ret = cw_field_ex_value(Value, State.Type)
...
;
PRO cw_field_ex_int, Ch, State, Event, Altered
...
FUNCTION cw_field_ex_exponent, Value, Idx
...
;
PRO cw_field_ex_float, Ch, State, Event, Altered
    ...
    IF Ch EQ 10b THEN BEGIN
    Value   = cw_field_ex_validate(Value)
    ...
    46b:    BEGIN
        ...
        IF cw_field_ex_exponent( Value, Idx ) THEN BEGIN
    ...
    END
    45b:    BEGIN
        HaveExp = cw_field_ex_exponent( Value, Idx )
...
function cw_field_ex_get_check, base
    StateHolder = widget_info(base, /CHILD)
    widget_control, StateHolder, get_uvalue=state

    if state.checkID eq -1 then return, -1
    return, widget_info(state.checkID, /button_set)
end

FUNCTION cw_field_ex_event, Event
    ...
    if( tag_names(Event, /structure_name) eq 'WIDGET_BUTTON' )then begin
      but = state.checkID
      butname = widget_info(but, /uname)
      textname = strmid(butname, 0, strlen(butname) - 7) + '_text'
      panel = widget_info(event.top, find_by_uname = textname)
      res = widget_info(but, /button_set)
      widget_control, panel, sensitive = widget_info(but, /button_set)
    ...
    IF Event.Type LE 1 THEN BEGIN       ; Insert Characters?
        ...
        IF State.Type EQ 1 THEN Procedure='cw_field_ex_float' $
        ELSE Procedure='cw_field_ex_int'
    ...
    WIDGET_CONTROL, State.TextId, GET_VALUE=Value
    RValue  = cw_field_ex_value(Value, State.Type)
...
;   FLOOR=vmin, CEILING=vmax
    ...
    hasCheckBox = keyword_set(optionBox)
    ...
    IF N_ELEMENTS(uname) EQ 0 THEN uname='cw_field_ex_uname'
    if n_elements(title_size) eq 0 then title_size = 50
    if n_elements(unit_text) eq 0 then unit_text = ''
    ...
    Base    = WIDGET_BASE(Parent, /Row, UVALUE=UValue, $
            EVENT_FUNC='cw_field_ex_event', $
            PRO_SET_VALUE='cw_field_ex_set', $
            FUNC_GET_VALUE='cw_field_ex_get', $
    ...
    if hasCheckBox then begin
      check = widget_base(base, /row, /nonexclusive)
      checkButton = widget_button(check, value = Title, $ 
                  xsize = title_size - 19, $  ; 19 == size of the checkbox
                  uname = uname + '_button')
    endif else begin
    ...
    ulabel = -1
    if strlen(unit_text) gt 0 then $
      ulabel = widget_label(base, value = unit_text, font = label_font)
      
    ; basic initialization
    if hasCheckBox then begin
      if keyword_set(optionOn) then widget_control, checkButton, /set_button $
      else widget_control, text, sensitive = 0
