function tmsat_get_fit_window, i, ienvi, nb, win, wfit, yfit, win_thresh
  m1 = i - win[ienvi]
  m2 = i + win[ienvi] + 1
  winmax = max(win)
...
function tmsat_savgol, y, w, win, forceUpperEnvelope, lastIterationLikeTIMESATfit
  ...
  for ienvi = 0, nenvi - 1 do begin
    ...
    for i = winmax, nb + winmax - 1 do begin
      ; set fitting window
      m1m2 = tmsat_get_fit_window(i, ienvi, nb, win, wfit, yfit, win_thresh)
      m1 = m1m2[0]
      m2 = m1m2[1]
      failleft = m1m2[2]
      failright = m1m2[3]
