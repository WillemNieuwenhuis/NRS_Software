function nrs_correlate_avg_get_julian_dates, event
  compile_opt idl2, logical_predicate, hidden
  
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_start_date')
  widget_control, val_fld, get_value = str_sd

  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_end_date')
...
pro nrs_correlate_avg_profile_handle_input, event
  compile_opt idl2, logical_predicate
  
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_refstack')
  widget_control, val_fld, get_value = target

  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_sens_perc')
  widget_control, val_fld, get_value = percent
  ...
  range = nrs_correlate_avg_get_julian_dates(event)
...
pro nrs_correlate_avg_profile_handleOK, event
  compile_opt idl2, logical_predicate
  ...
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_table')
  ...
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_sens_iter')
  widget_control, val_fld, get_value = iterations

  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_sens_perc')
  widget_control, val_fld, get_value = percent
  
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_NAN_button')
  ignore_NAN = widget_info(val_fld, /button_set)

  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_from_date')
  widget_control, val_fld, get_value = from_date
  
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_to_date')
  widget_control, val_fld, get_value = to_date
  
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_from_band')
  widget_control, val_fld, get_value = from_band
  
  val_fld = widget_info(event.top, find_by_uname = 'nrs_correlate_avg_profile_to_band')
  ...
  if n_elements(iterations) eq 0 then begin
    void = error_message('Number of iterations not specified', traceback = 0, /error)
    return
  endif
  iterations = long(iterations[0])
  if iterations lt 1 then begin
    void = error_message('Number of iterations cannot be negative', traceback = 0, /error)
    return
  endif
  
  if n_elements(percent) eq 0 then begin
    void = error_message('Sample percentage not specified', traceback = 0, /error)
    return
  endif
  percent = fix(percent[0])
  if (percent le 1) || (percent ge 99) then begin
    void = error_message('Sample percentage must be between 1% and 99%', traceback = 0, /error)
    return
  endif
  ...
  prog_obj = Obj_New("PROGRESSBAR", background = 'white', color = 'green' $
                        , ysize = 15, title = "Profile correlation sensistivity" $
                        , /fast_loop $
                        )
  
  nrs_correlate_avg_profile, ref, table $
                       , iterations = iterations $
                       , select_perc = float(percent) / 100.0 $
