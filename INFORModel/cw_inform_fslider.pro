...
;
; CALLING SEQUENCE:
;   widget = CW_INFORM_FSLIDER(Parent)
;
; INPUTS:
...
;	March 2008, WN, ITC, Moved the slider after the text box in horizontal mode
;-
...
function fslide_event, ev
  ...
  if (*pState).efun eq '' then $
    ...
    CALL_PROCEDURE, (*pState).efun, ret
    return, ret
end
...
pro cw_inform_fslider_kill_notify, wChild
...
;-----------------------------------------------------------------------------
function cw_inform_fslider, parent, $
        DOUBLE = doubleIn, $
        DRAG = drag, $
        EDIT = edit, $
        EVENT_PRO = efun, $
  ...
  uname = (N_ELEMENTS(unameIn) gt 0) ? unameIn : 'cw_inform_FSLIDER_UNAME'
  ...
  if (sup eq 0) then begin
    (*pState).labelid = WIDGET_TEXT(labelbase, YOFFSET = l_yoff, $
                edit=edit, UNAME=uname+'_TEXT', xsize = 10)
  endif
  ...
  WIDGET_CONTROL, wChild, $
    ...
    KILL_NOTIFY='cw_inform_fslider_kill_notify'
