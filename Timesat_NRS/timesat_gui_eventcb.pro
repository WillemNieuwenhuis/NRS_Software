pro tmsat_handleBrowseInput, event
  fld = widget_info(event.top, find_by_uname = 'tmsat_input_text')
...
pro tmsat_handleGo, event
  compile_opt idl2, logical_predicate

	; Collect all input, parameters and output from UI
	val_fld = widget_info(event.top, find_by_uname = 'tmsat_input_text')
	widget_control, val_fld, get_value = src_filename

	val_fld = widget_info(event.top, find_by_uname = 'tmsat_output_text')
	widget_control, val_fld, get_value = out_filename

  val_fld = widget_info(event.top, find_by_uname = 'tmsat_range_min')
  widget_control, val_fld, get_value = vr_min_str

  val_fld = widget_info(event.top, find_by_uname = 'tmsat_range_max')
  widget_control, val_fld, get_value = vr_max_str

	val_fld = widget_info(event.top, find_by_uname = 'tmsat_SavGolText')
  ...
  if strlen(strtrim(vr_max_str, 2)) gt 0 then vr_max = float(vr_max_str[0])
	...
	val_fld = widget_info(event.top, find_by_uname = 'tmsat_UpEnvelCheck')
  ...
  progressBar->Start
	...
	timesat_v11, src_filename, out_filename $
