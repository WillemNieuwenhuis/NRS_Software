; $Id: read_csv.pro 1356 2011-03-08 10:56:44Z windowsnt\nieuwenhuis $
;
...
;   Adjusted to keep date / time string as string, Nieuwenhuis, ITC, dec 2011  
;-
function nrs_read_csv, Filename, $
  COUNT=count, $
  HEADER=header, $
  ...
  doDS = ~(n_elements(date_sep) eq 0 || strlen(date_sep) eq 0) 
  doTS = ~(n_elements(time_sep) eq 0 || strlen(time_sep) eq 0) 
  if (count gt 1) then begin
    ...
    for j=0,nColumns-1 do begin
      ...
      if doTS then begin 
        tml = strsplit(subdata, time_sep, count = tml_cnt)
        hasColon = max(tml_cnt) gt 1
      endif else hasColon = 0
      if doDS then begin
        dtl = strsplit(subdata, date_sep, count = dtl_cnt)
        hasDash = max(dtl_cnt) gt 1
      endif else hasDash = 0
      
      if hasDash || hasColon then continue  ; keep date or time as string
